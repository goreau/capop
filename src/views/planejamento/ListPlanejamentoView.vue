...
export default {
  ...
  mounted() {
   ...
   // this.myspan.innerHTML='<p>teste</p>';;
      ...
      this.isLoading = true;
      planejamentoService.getPlanejamentos(this.id_user)
          .then((response) => {
      ...
      this.columns = [
          ...
          {title: 'Local', field: 'municipio', type: 'string', minWidth: 250},
          ...
          {title: 'Ações', responsive: 0, minWidth: 250, 
            formatter: (cell, formatterParrams) =>{
              ...
              btEdit.addEventListener('click', () => {
                this.$router.push(`/editPlanejamento/${row.id_planejamento}`);
              });
            ...
            /* const teste = document.createElement('div'); 
              ...
              if (ok) {
                planejamentoService.delete(row.id_planejamento)
                .then(resp =>{
                  if (resp.status == '200'){
                    location.reload();
                  } else {
